  ...
  return <div className="editor-layout ui-editor-layout">
    <section className="editor-main">
      ...
      <div className="stage-scroll">
        <div ref={stageRef} className="ui-stage" style={{ width: ui.screen.width, height: ui.screen.height, transform:`scale(${zoom})`, transformOrigin:'top left', backgroundColor:ui.screen.backgroundColor, backgroundImage:ui.screen.asset&&assetUrls?.__screenBackground?`url(${assetUrls.__screenBackground})`:undefined, backgroundSize:ui.screen.assetFit==='cover'?'cover':ui.screen.assetFit==='contain'?'contain':'100% 100%', backgroundPosition:'center', backgroundRepeat:'no-repeat', '--stage-zoom':zoom }} onPointerDown={()=>setSelectedId('')}>
          ...
          {ordered.map((el)=>{
            ...
            return <div key={el.id} className={`ui-element-preview ${selectedNow?'selected':''} ui-${el.type}`} style={{left:t.x,top:t.y,width:t.width,height:t.height,zIndex:t.z,background:el.style?.background,color:el.style?.color,fontSize:el.style?.fontSize}} onPointerDown={(e)=>{e.stopPropagation();beginDrag(e,el)}}>
