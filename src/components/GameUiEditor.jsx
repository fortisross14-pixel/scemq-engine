...
export default function GameUiEditor({ ui, assetUrls, onChange, onChooseAsset, onChooseScreenBackground, onClearScreenBackground, onChooseCursor, onImport, onExport }) {
  ...
  const fallbackGuiTop = (ui.viewport?.y || 0) + (ui.viewport?.height || 0);
  const guiBackground = ui.screen?.guiBackground || { x:0, y:fallbackGuiTop, width:ui.screen.width, height:Math.max(0, ui.screen.height - fallbackGuiTop) };
  function patchGuiBackground(patch) { onChange({ ...ui, screen:{ ...ui.screen, guiBackground:{ ...guiBackground, ...patch } } }); }
  ...
  return <div className="editor-layout ui-editor-layout">
    <section className="editor-main">
      ...
      <div className="stage-scroll">
        <div ref={stageRef} className="ui-stage" style={{ width: ui.screen.width, height: ui.screen.height, transform:`scale(${zoom})`, transformOrigin:'top left', backgroundColor:ui.screen.backgroundColor, '--stage-zoom':zoom }} onPointerDown={()=>setSelectedId('')}>
          {ui.screen.asset&&assetUrls?.__screenBackground&&guiBackground.width>0&&guiBackground.height>0?<div className="ui-bottom-skin-preview" style={{left:guiBackground.x,top:guiBackground.y,width:guiBackground.width,height:guiBackground.height}}><img src={assetUrls.__screenBackground} alt="" draggable="false" style={{objectFit:ui.screen.assetFit==='cover'?'cover':ui.screen.assetFit==='contain'?'contain':'fill'}}/></div>:null}
    ...
    <aside className="inspector">
      ...
      {!selected ? <>
        ...
        <div className="inspector-divider"/><div className="inspector-subtitle">Bottom GUI background image</div><div className="asset-path">{ui.screen.asset||'No image assigned'}</div><div className="toolbar-group"><button className="wide-button" onClick={onChooseScreenBackground}>Choose background image</button>{ui.screen.asset&&<button className="icon-button" onClick={onClearScreenBackground}>×</button>}</div><InspectorField label="Image fit"><select value={ui.screen.assetFit||'stretch'} onChange={(e)=>onChange({...ui,screen:{...ui.screen,assetFit:e.target.value}})}><option value="stretch">stretch</option><option value="cover">cover</option><option value="contain">contain</option></select></InspectorField><div className="transform-grid">{['x','y','width','height'].map(k=><InspectorField key={k} label={k.toUpperCase()}><input type="number" value={guiBackground[k]} onChange={e=>patchGuiBackground({[k]:Math.max(k==='width'||k==='height'?0:-9999,Number(e.target.value))})}/></InspectorField>)}</div><button className="wide-button" onClick={()=>patchGuiBackground({x:0,y:fallbackGuiTop,width:ui.screen.width,height:Math.max(0,ui.screen.height-fallbackGuiTop)})}>Fit exactly below viewport</button><div className="linked-note">The background has its own editable container. Fit controls how the image sits inside that rectangle; X/Y/Width/Height control the rectangle itself.</div>
