...
export default function GameUiEditor({ ui, assetUrls, onChange, onChooseAsset, onChooseCursor, onImport, onExport }) {
  ...
  return <div className="editor-layout ui-editor-layout">
    <section className="editor-main">
      ...
      <div className="stage-scroll">
        <div ref={stageRef} className="ui-stage" style={{ width: ui.screen.width, height: ui.screen.height, transform:`scale(${zoom})`, transformOrigin:'top left', background:ui.screen.backgroundColor, '--stage-zoom':zoom }} onPointerDown={()=>setSelectedId('')}>
          ...
          {ordered.map((el)=>{
            const t=el.transform; const selectedNow=el.id===selectedId; const assetUrl=assetUrls?.[el.id];
            return <div key={el.id} className={`ui-element-preview ${selectedNow?'selected':''} ui-${el.type}`} style={{left:t.x,top:t.y,width:t.width,height:t.height,zIndex:t.z,background:el.style?.background,color:el.style?.color,fontSize:el.style?.fontSize}} onPointerDown={(e)=>{e.stopPropagation();beginDrag(e,el)}}>
              {el.type==='image' && assetUrl ? <img src={assetUrl} alt="" draggable="false"/> : null}
              ...
              {el.type==='statusText' ? <span>Use key with door</span> : el.type!=='inventory' && el.type!=='image' ? <span>{el.label || el.name}</span> : null}
    ...
    <aside className="inspector">
      ...
      {!selected ? <>
        ...
        <div className="linked-note">This rectangle is the portion of every scene visible during play. UI elements stay fixed outside or on top of it.</div><div className="inspector-divider"/><div className="inspector-subtitle">Verb cursors (optional)</div><div className="cursor-grid">{Object.keys(ui.cursors||{}).map(verb=><div className="cursor-row" key={verb}><span>{verb}</span><code>{ui.cursors[verb]||'default cursor'}</code><button onClick={()=>onChooseCursor(verb)}>Choose</button></div>)}</div>
      </> : <>
